...
class NNEvaluator : public Evaluator {
  struct ConvBlock : nn::Module {
    ConvBlock() {
      conv1 = register_module(
          "conv1", nn::Conv2d(nn::Conv2dOptions(5, NUM_FILTERS, 3)
                                         .stride(1)
                                         .padding(1)));
      bn1 = register_module(
  ...
  struct ResBlock : nn::Module {
    ResBlock() {
      conv1 = register_module("conv1", nn::Conv2d(nn::Conv2dOptions(NUM_FILTERS, NUM_FILTERS, 3).stride(1).padding(1).bias(false)));
      bn1 = register_module("bn1", nn::BatchNorm2d(NUM_FILTERS));
      conv2 = register_module("conv2", nn::Conv2d(nn::Conv2dOptions(NUM_FILTERS, NUM_FILTERS, 3).stride(1).padding(1).bias(false)));
  ...
  struct OutBlock : nn::Module {
    OutBlock() {
      ...
      p_fc1 = register_module("p_fc1", nn::Linear(BOARD_SIZE * BOARD_SIZE * 2, BOARD_SIZE * BOARD_SIZE + 1));

      v_conv1 = register_module("v_conv1", nn::Conv2d(NUM_FILTERS, 1, 1));
      v_bn1 = register_module("v_bn1", nn::BatchNorm2d(1));
      v_fc1 = register_module("v_fc1", nn::Linear(BOARD_SIZE * BOARD_SIZE, NUM_FILTERS));
  ...
  struct Net : nn::Module {
    // sequential is used to register each block, etc.; not sure if better way to do this
    Net() {
